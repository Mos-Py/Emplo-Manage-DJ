...
def export_person_salary_excel(request, person_id):
    ...
    try:
        ...
        loan_remaining = 0
        if hasattr(Fee, 'fee_status') and hasattr(Fee, 'remaining'):
            active_loans = Fee.objects.filter(person=person, fee_status=1, remaining__gt=0)
            loan_remaining = sum((float(l.remaining) for l in active_loans), 0.0)
