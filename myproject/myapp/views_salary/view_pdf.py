...
from urllib.parse import quote
from django.http import HttpResponse
from .view_excel import export_person_salary_excel
...
def export_person_salary_pdf(request, person_id):
    try:
        ...
        # เรียกใช้งาน Excel รายคน (ไม่มีแทนค่า)
        excel_response = export_person_salary_excel(request, person_id)
        if excel_response.status_code != 200:
            return HttpResponse("Export Excel ล้มเหลว", status=500)

        temp_dir = tempfile.gettempdir()
        excel_path = os.path.join(temp_dir, f"person_{person_id}_{int(time.time())}.xlsx")
        ...
        filename = f"สลิปเงินเดือน_{month_name}_{year_thai}_{person.first_name}.pdf"
