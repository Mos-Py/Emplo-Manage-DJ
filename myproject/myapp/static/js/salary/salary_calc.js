...
// อัปเดตข้อมูลเงินกู้
// อัปเดตฟังก์ชันเดิมให้ตรวจสอบยอดเงิน
function updateLoanPayment() {
    ...
    const loanRemaining = parseFloat(document.getElementById("loanRemainingAmount")?.innerText || 0);
    
    if (loanPayment > loanRemaining) {
        // แสดงข้อความแจ้งเตือนใต้ช่องกรอก
        let errorMsg = document.getElementById("loan-payment-error");
        if (!errorMsg) {
            errorMsg = document.createElement("div");
            errorMsg.id = "loan-payment-error";
            errorMsg.className = "text-danger small mt-1";
            loanPaymentInput.parentNode.appendChild(errorMsg);
        }
        errorMsg.innerText = "ยอดชำระเกินยอดคงเหลือ!";
        
        // ปรับสีขอบช่องกรอกให้เป็นสีแดง
        loanPaymentInput.classList.add("border-danger");
    } else {
        // ลบข้อความแจ้งเตือนถ้ามี
        const errorMsg = document.getElementById("loan-payment-error");
        if (errorMsg) {
            errorMsg.remove();
        }
        
        // ปรับสีขอบช่องกรอกกลับเป็นปกติ
        loanPaymentInput.classList.remove("border-danger");
    }
    
    // เก็บค่าไว้ในฟิลด์ hidden
    document.getElementById("loanPaymentValue").value = loanPayment;
    
    // คำนวณยอดรวมใหม่
    recalculateNetTotal();
}

// เมื่อโหลด DOM เสร็จ ให้เพิ่ม event listener
document.addEventListener("DOMContentLoaded", function() {
    const loanPaymentInput = document.getElementById("loanPaymentInput");
    if (loanPaymentInput) {
        // เพิ่ม event listener แค่ครั้งเดียว
        loanPaymentInput.addEventListener("input", updateLoanPayment);
    }
});
