...
function checkAndShowLoanInfo(personId, salaryData) {
    ...
    // เปลี่ยนตรงนี้ - ตรวจสอบจาก API แทนที่จะเช็คจาก salaryData
    fetch(`/api/loan_summary/${personId}/?month=${selectedMonth}`)
        ...
        .then(loanData => {
            ...
            // แสดงข้อมูลเงินกู้ถ้ามียอดคงเหลือมากกว่า 0
            if (loanData.loan_remaining > 0) {
                console.log("พบข้อมูลเงินกู้คงเหลือ:", loanData.loan_remaining);
                
                if (loanMonthlyLineEl) {
                    loanMonthlyLineEl.style.display = "flex";
                }
                
                if (loanPaymentInputEl) {
                    // ใช้ค่าจาก loan_paid_this_month หรือ salaryData.loan_payment
                    const paymentValue = salaryData.loan_payment || loanData.loan_paid_this_month || 0;
                    loanPaymentInputEl.value = paymentValue.toFixed(2);
                }
                
                if (loanPaymentValueEl) {
                    const paymentValue = salaryData.loan_payment || loanData.loan_paid_this_month || 0;
                    loanPaymentValueEl.value = paymentValue.toFixed(2);
                }
                
                if (loanRemainingLineEl) {
                    loanRemainingLineEl.style.display = "flex";
                }
                ...
                console.log("ไม่พบข้อมูลเงินกู้คงเหลือ");
